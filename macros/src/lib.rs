        ...
        impl ::nimiq_macros::serde::Serialize for $name {
                ...
                S: ::nimiq_macros::serde::Serializer,
            {
                if serializer.is_human_readable() {
                    ::nimiq_macros::serde::Serialize::serialize(&::nimiq_macros::hex::encode(&self.0), serializer)
                } else {
        ...
        impl<'de> ::nimiq_macros::serde::Deserialize<'de> for $name {
                ...
                D: ::nimiq_macros::serde::de::Deserializer<'de>,
            {
                let data: [u8; $len] = if deserializer.is_human_readable() {
                    let s: Cow<'de, str> = ::nimiq_macros::serde::Deserialize::deserialize(deserializer)?;
                    ::nimiq_macros::hex::decode(&s[..])
                        .map_err(|_| <D::Error as ::nimiq_macros::serde::de::Error>::custom("Could not parse hex string"))?
                        .try_into()
                        .map_err(|_| <D::Error as ::nimiq_macros::serde::de::Error>::custom("Could not parse hex string: invalid length"))?
                } else {
