...
impl Client {
    ...
    pub async fn get_transactions_by_address(
        ...
        if let Some(array) = known_transaction_details {
            ...
            for receipt in known_transaction_details {
                known_txs.insert(receipt.transaction.transaction_hash.clone(), receipt);
            }
        }
        ...
        for (hash, block_number) in receipts.iter() {
            ...
            if let Some(known_tx) = known_txs.get(&hash.to_hex()) {
                // TODO: Instead of block height, compare block hash when available in receipt and known tx
            ...
            // Check all receipts that are newer than since_block_height.
        ...
        // received block height to not re-request cut-off known transactions.
        ...
        for details in known_txs.values() {
            ...
            if details.block_height.unwrap() < since_block_height {
                continue;
            }
            let hash = Blake2bHash::from_str(&details.transaction.transaction_hash).unwrap();
            if receipts.contains_key(&hash) {
                continue;
            }

            receipts_to_fetch.push((hash, details.block_height));
        }
